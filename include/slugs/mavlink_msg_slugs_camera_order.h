...
typedef struct __mavlink_slugs_camera_order_t 
{
	...
	int8_t zoom; ///< Order the camera to zoom: -1 zoom out, 0 No zoom change, +1 zoom in
 ...
 */
static inline uint16_t mavlink_msg_slugs_camera_order_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint8_t target, int8_t pan, int8_t tilt, int8_t zoom, int8_t moveHome)
{
	uint16_t i = 0;
	msg->msgid = MAVLINK_MSG_ID_SLUGS_CAMERA_ORDER;

	i += put_uint8_t_by_index(target, i, msg->payload); // The system reporting the action
	i += put_int8_t_by_index(pan, i, msg->payload); // Order the mount to pan: -1 left, 0 No pan motion, +1 right
	i += put_int8_t_by_index(tilt, i, msg->payload); // Order the mount to tilt: -1 down, 0 No tilt motion, +1 up
	i += put_int8_t_by_index(zoom, i, msg->payload); // Order the camera to zoom: -1 zoom out, 0 No zoom change, +1 zoom in
	i += put_int8_t_by_index(moveHome, i, msg->payload); // Orders the camera mount to move home. The other fields are ignored when this field is set. 1: move home, 0 ignored
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_slugs_camera_order_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint8_t target, int8_t pan, int8_t tilt, int8_t zoom, int8_t moveHome)
{
	uint16_t i = 0;
	msg->msgid = MAVLINK_MSG_ID_SLUGS_CAMERA_ORDER;
